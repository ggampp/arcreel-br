...
Este projeto é **modo narração + imagem** (`narration`). A estrutura de dados do script é `segments[]`; cada segmento corresponde a um trecho de leitura em voz alta + uma imagem de storyboard.
...
Agent principal (camada de orquestração — extremamente leve)
  ...
  └─ dispatch → generate-assets              geração de ativos (personagem/cena/prop/storyboard/vídeo/narração TTS)
```
...
| generate-narration-audio | `/generate-narration-audio` | Gera narração (TTS por segmento, só depende de novel_text do script) |
...
9. **Narração TTS**: só modos `storyboard` / `grid` (`reference_video` sem segments, pula) → dispatch do subagent `generate-assets` (TTS por segmento; só depende de `novel_text` do script, independente do vídeo; pode avançar assim que o script existir)
...
├── audio/             # áudio de narração (modo narration, criado na primeira geração)
