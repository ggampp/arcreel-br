...
2. Use Read em `project.json` e confirme os campos `title`, `content_mode`, `generation_mode` (nesta sessão o content_mode atual é `drama`, imutável após a criação)
...
3. O episódio-alvo tem `ledger_status` `stale` (invalidado após reordenação — step1/script/mídia antigos contam como inválidos; mesmo com arquivos ainda presentes, refaça a partir desta etapa; produtos são substituídos pelo mecanismo de versão), ou o arquivo intermediário step1 da **combinação atual** do episódio-alvo não existe? → **Etapa 3**. Verifique o arquivo correspondente nos três ramos `effective_mode(project, episode)` × `content_mode` (note que effective_mode inclui o override `episodes[i].generation_mode` do episódio; não olhe só o campo de topo do projeto):
   ...
   - effective_mode ∈ {storyboard, grid} e content_mode == drama: `drafts/episode_{N}/step1_normalized_script.json` (conteúdo estruturado, ver ADR 0041)

   Neste projeto content_mode é fixo em drama (imutável após a criação), então só acerta o 1º ou o 3º ramo, conforme o effective_mode do episódio. Só reconheça o arquivo da combinação atual: **outros `step1_*` de outros modos** no diretório são resíduo e não contam como etapa 3 concluída. `step1_normalized_script.md` residual de projetos antigos (rascunho livre pré-estruturação) também não conta como step1 válido — se não houver `.json`, rode de novo `normalize-drama-script` na etapa 3 para produzir `.json` estruturado; não faça migração md→estruturado.
...
8. Tudo concluído → fim do fluxo; oriente o usuário a exportar o rascunho CapCut/Jianying no Web
...
- caso contrário (neste projeto content_mode == drama) → dispatch `normalize-drama-script` (produz conteúdo estruturado `drafts/episode_{N}/step1_normalized_script.json`)
