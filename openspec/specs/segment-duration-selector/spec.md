...
O elemento de exibição de duração em `ShotDetail` SHALL permitir que o usuário clique e abra um seletor para alternar entre `durationOptions` (lista de durações opcionais resolvida pelas capacidades do backend de vídeo); após a seleção, o novo valor é gravado no backend via callback `onUpdatePrompt` e, ao concluir o salvamento, a duração total do episódio é atualizada. Quando `durationOptions` for um intervalo contínuo de inteiros com muitas opções, o seletor é apresentado como slider.

#### Scenario: Clicar no badge de duração abre o seletor

- **WHEN** o usuário clica no badge de duração (ex.: "4s")
- **THEN** abre um Popover listando botões (ou slider) conforme `durationOptions`, com o valor atual em destaque
...
- **WHEN** a prop `onUpdatePrompt` não é fornecida (modo somente leitura)
