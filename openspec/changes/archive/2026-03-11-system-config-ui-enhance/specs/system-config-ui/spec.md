...
### Requirement: Estrutura de quatro Tabs na barra superior

A página de configuração SHALL expandir as Tabs da barra superior de `[config, api-keys]` para quatro Tabs: **Configuração do agente ArcReel** (agent), **Configuração de geração de imagem/vídeo IA** (media), **Configuração avançada** (advanced), **API Keys** (api-keys).

Conteúdo de cada Tab:
- **Configuração do agente ArcReel**: Anthropic API Key, Base URL, campos de seleção de modelos
...
- **API Keys**: funcionalidade existente de gerenciamento de API Key (inalterada)

#### Scenario: Usuário abre a página de configuração do sistema
- **WHEN** o usuário navega para `/app/settings`
- **THEN** a página SHALL exibir as quatro Tabs na barra superior, com a primeira Tab (Configuração do agente ArcReel) ativa por padrão

#### Scenario: Ordem das Tabs
- **WHEN** a página renderiza
- **THEN** a ordem das Tabs SHALL ser fixa: Configuração do agente ArcReel → Configuração de geração de imagem/vídeo IA → Configuração avançada → API Keys

---

### Requirement: Salvamento independente por Tab

Cada Tab de configuração (agent / media / advanced) SHALL oferecer operação de salvamento independente, salvando de uma vez todos os campos modificados da Tab, sem afetar outras Tabs.

#### Scenario: Usuário modifica campos na Tab e salva
- **WHEN** o usuário modifica qualquer campo de uma Tab de configuração e clica no botão salvar da Tab
- **THEN** o sistema SHALL fazer PATCH de todos os campos modificados da Tab; após sucesso, a Tab volta ao estado não modificado

#### Scenario: Estado de salvamento em andamento na Tab
- **WHEN** a requisição de salvamento da Tab está em andamento
- **THEN** todos os campos de entrada da Tab SHALL ser desabilitados e o botão salvar exibe estado de carregamento, evitando envio duplicado

#### Scenario: Falha no salvamento da Tab
...
- **WHEN** todos os valores de campo da Tab são iguais aos valores já salvos
